...
exports.getOne = async ctx => {
  try {
    ...
    let dataCall = await api.get(`${contact.address.city},${contact.address.state}`);
    let textMessage = '';
    if (dataCall.temp == 18 || dataCall.temp < 18) textMessage = 'Você não gostaria de um chocolate quente?';
    if (dataCall.temp >= 30 && dataCall.condition_code == 32) textMessage = 'Recomendo fortemente você ir à praia com esse calor!';
    if (dataCall.temp >= 30 && (dataCall.condition_code == 45 || dataCall.condition_code == 9)) textMessage = 'Um sorvete agora não seria nada mal!';
    if (dataCall.temp < 30 && dataCall.temp > 18 && (dataCall.condition_code == 32)) textMessage = 'Com um clima desse, uma atividade ao ar livre seria bom!';
    if (dataCall.temp < 30 && dataCall.temp > 18 && (dataCall.condition_code == 45 || dataCall.condition_code == 9)) textMessage = 'O clima de hoje sugere ver um bom filme!';
    ctx.assert(contact, STATUS_CODE.NOT_FOUND, "The requested contact doesn't exist");
    ctx.status = 200;
...
exports.getAll = async ctx => {
  ...
  try {
    if (text) {
      const users = await Contacts.find({ "$or": [
...
exports.deleteOne = async ctx => {
  ...
  try {
    const contact = await Contacts.findOneAndUpdate(contactId, {
      $set: { deleted: true }
    }, { new: true });
    ctx.status = STATUS_CODE.OK;
    ctx.body = contact;
