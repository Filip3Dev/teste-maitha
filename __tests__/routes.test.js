...
describe('basic route tests', () => {
    ...
    test('GET one contact', async () => {
        ...
        const response2 = await request(server.callback()).get(`/api/v1/contact/${body[0]._id}`);
        expect(response2.status).toBe(200);
        expect(response2.body).toBeInstanceOf(Object);
    });
    test('EDIT one contact', async () => {
        const { body } = await request(server.callback()).get('/api/v1/contact/');
        const response2 = await request(server.callback()).put(`/api/v1/contact/${body[0]._id}`, body[0]);
        expect(response2.status).toBe(200);
        expect(response2.body).toBeInstanceOf(Object);
    });
    test('DELETE one contact', async () => {
        const { body } = await request(server.callback()).get('/api/v1/contact/');
        const response2 = await request(server.callback()).delete(`/api/v1/contact/${body[0]._id}`);
        expect(response2.status).toBe(200);
        expect(response2.body).toBeInstanceOf(Object);
    });
});